...
from data_modules.token_utterance_masking import UtteranceMaskingDataModule as TokenUtteranceMaskingDataModule
from data_modules.binary_utterance_masking import UtteranceMaskingDataModule as BinaryUtteranceMaskingDataModule
from models.lite_gpt2_dh import LiteGPT2DoubleHeadsModel
# from utils.callbacks import GenerateResponseAndPushModelToHubCallback
...
def main():
    ...
    parser.add_argument("--masked_prob_ratio", type=float, default=0.8)
    parser.add_argument("--changed_prob_ratio", type=float, default=0.5)
    ...
    if args.output_dir:
        app_name = f"{args.run_name}_{args.do_type.lower()}_alpha-{args.alpha}_do_prob-{args.do_prob_ratio}_masked_prob-{args.masked_prob_ratio}_changed_prob-{args.changed_prob_ratio}_{now}"
    ...
    if args.do_type == "TOKEN":
        ...
        dm = TokenUtteranceMaskingDataModule(
            tokenizer=tokenizer,
            ...
            masked_prob=args.masked_prob_ratio,
            changed_prob=args.changed_prob_ratio,
        )
    else:
        features_kwargs["zero_keys"].append("aux_labels")
        dm = BinaryUtteranceMaskingDataModule(
            tokenizer=tokenizer,
            train_file=args.train_file,
            validation_file=args.validation_file,
            test_file=args.test_file,
    ...
    num_labels, aux_num_labels = 1, len(tokenizer) if args.do_type == "TOKEN" else 3
