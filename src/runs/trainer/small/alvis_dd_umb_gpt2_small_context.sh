#!/bin/env bash
#SBATCH --nodes=1
#SBATCH --account=xxxx-xx-xxx
#SBATCH --partition=energi
#SBATCH --time=0-12:00:00
#SBATCH --job-name=dd-umb-gpt2-small-context
#SBATCH --error=logs/dd-umb-context-%J.err.log
#SBATCH --output=logs/dd-umb-context-%J.out.log
#SBATCH --gpus-per-node=A100:1
#SBATCH --mail-type=ALL
#SBATCH --mail-user=your_uni_email

export SEED=101
export USERNAME_DIR=xxxx-xx-xxx
...
export RUN_NAME=${DATASET_MODE}-umb-gpt2-small-context
...
export PRETRAINED_MODEL_NAME_OR_PATH=gpt2

#--deepspeed_sharding
export DO_TYPE=BINARY
export DO_ON=CONTEXT
export ALPHA=3.0
# --do_prob_ratio=$MASKED_PROB_RATIO
# --masked_prob_ratio=$MASKED_PROB_RATIO
# --changed_prob_ratio=$REPLACED_PROB_RATIO
export DO_PROB_RATIO=0.15
export MASKED_PROB_RATIO=0.8
export CHANGED_PROB_RATIO=0.5
export LR=5e-5
export WEIGHT_DECAY=0.01
export NUM_WARMUP_STEPS=5000

export OUTPUT_DIR=/mimer/NOBACKUP/groups/${USERNAME_DIR}/OUTPUT/checkpoints/
export HUB_MODEL_ID=m3hrdadfi/${DATASET_MODE}-umb-gpt2-small-context
export HUB_TOKEN=xxxx-xx-xxx
export REPORT_TO=all
export WANDB_TOKEN=xxxx-xx-xxx
...
python run_trainer_utterance_masking.py \
  ...
  --masked_prob_ratio=$MASKED_PROB_RATIO \
  --changed_prob_ratio=$CHANGED_PROB_RATIO \
  --do_on="$DO_ON" \
  --do_type="$DO_TYPE" \
  --lr=$LR \
